...
How different would computing feel if important actions were authorized not down the chain of command, but rather through the presence of a quorum of equals? Say any five members of the community running a community datacenter have to connect their secure tokens in order to gain elevated privileges on the system. If the number falls below the required quota, the privileged session terminates. These physical tokens may even be embedded in a bracelet or necklace, as tangible artifacts that signify community.
